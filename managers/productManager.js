...
export default class ProductManager {
    ...
    addProduct = async (obj) => {
        ...
        if (title === undefined || description === undefined || price === undefined || category === undefined || status === undefined || code === undefined || stock === undefined) {
            console.error("Por favor ingrese todos los campos del producto")
            return
        }
        else {
            const listaproducts = await this.getProducts({})
            ...
            if (codigorepetido) {
                console.error("El producto que desea agregar se repite")
                return
            }
            else {
    ...
    updateProduct = async (id, obj) => {
        ...
        if (title === undefined || description === undefined || price === undefined || category === undefined || status === undefined || code === undefined || stock === undefined) {
            console.error("Por favor ingrese todos los campos para su correcta actualización")
            return
        }
        else {
            const allproducts = await this.getProducts({})
            ...
            if (codigorepetido) {
                console.error("El código de producto que desea actualizar se repite")
                return
            }
            else {
